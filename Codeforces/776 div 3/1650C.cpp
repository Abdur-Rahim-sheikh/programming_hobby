...
int main(){
    ...
    while(test--){
        cin>>n>>m;
        vector<pair<pii,int>> arr(m);
        for(i=0;i<m;i++){
            cin>> arr[i].x.first>> arr[i].x.second;
            arr[i].y= i+1;
        }
        sort(all(arr),comp1);
        sort(arr.begin(),arr.begin()+2*n,comp2);
        int sum=0;
        for(i=0;i<2*n;i++)sum+=arr[i].x.second;
        cout<<sum<<endl;
        for(i=0;i<n;i++){
            cout<<arr[i].y<<' '<<arr[2*n-i-1].y<<endl;
        }
        cout<<endl;
    }
